...
if doesFileExist(LOGS_PATH) then
	...
	if file then
		my_logs = decodeJson(file:read("*a")) or {}
		file:close()
	end
end
function save_logs()
...
auto_update = {
	ui_scale = 100,
	last_update = 0,
	website = "https://disk.yandex.ru/d/JMMhXOX81azjpA?dl=1", -- URL Яндекс диска для получения цен
	attempting = false,
	sound_enabled = true,
...
local SCRIPT_UPDATE_URL = "https://raw.githubusercontent.com/Likyor/AutoBazar/refs/heads/main/new_ab.lua" -- Ссылка на скачивание самого .lua скрипта (заменишь позже)
...
local function showCefNotification(text, isAuction)
	...
	lua_thread.create(function()
		-- Ждём нулевой кадр, чтобы отсеять массовый спам при прогрузке АБ
		wait(0)
		if cefNotifyCounter ~= currentId then return end
		
		-- Закрываем старое уведомление, если оно всё ещё висит
		...
		-- Ждём один кадр (50 мс), чтобы CEF успел отработать закрытие
		wait(50)
		if cefNotifyCounter ~= currentId then return end
...
function checkUpdates(manual)
	lua_thread.create(function()
		if manual then chatMessage("Проверка обновлений...") end
		local response = fetchUrl(UPDATE_JSON_URL)
		if response and response ~= "" then
			local data = decodeJson(response)
			if data and data.version and data.author then
				...
				if new_ver and new_ver > current_ver then
					if data.author == "Sunshinek" or data.author == "Likyor" then
						...
						updateUrl = data.url or "https://www.blast.hk/threads/253072/"
						if data.script_url then SCRIPT_UPDATE_URL = data.script_url end
						if manual then
							chatMessage("Найдено обновление до версии {FF6640}%s{EEEEEE}!", data.version)
							uiShowUpdates[0] = true
							uiShowLogs[0] = false
						else
							wait(3000)
							showCefNotification(string.format("Доступна новая версия <b>%s</b> от <b>%s</b>! Проверьте вкладку 'Обновления'.", data.version, data.author), false)
							chatMessage("Доступно обновление скрипта до версии {FF6640}%s{EEEEEE} от автора {FF6640}%s{EEEEEE}!", data.version, data.author)
						end
					end
				else
					if manual then chatMessage("У вас установлена последняя версия скрипта.") end
				end
			elseif manual then
				chatMessage("Ошибка обработки данных обновления.")
			end
		elseif manual then
			chatMessage("Не удалось проверить обновления. Проверьте интернет-соединение.")
		end
	end)
end
...
function performUpdate(url)
	lua_thread.create(function()
		chatMessage("Скачивание обновления...")
		...
		if response and response ~= "" then
			...
			if f then
				f:write(response)
				f:close()
				chatMessage("Обновление успешно скачано! Перезагрузка скрипта...")
				wait(1000)
				thisScript():reload()
			else
				chatMessage("Ошибка записи файла обновления.")
			end
		else
			chatMessage("Ошибка скачивания файла обновления.")
		end
	end)
end
...
imgui.OnFrame(function() return renderWindow[0] end, function(player)
	...
	if imgui.Begin("##АвтоБазарCEF", renderWindow, flags) then
		...
		if BeginCEFSection("##Settings", u8"Настройки", u8"ИНТЕРФЕЙС", imgui.ImVec2(right_w, content_h - 205 * scale), scale) then
			...
			imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, auto_update.rounding * 0.6 * scale)
			if imgui.Button(fa.ICON_FA_SYNC .. " " .. u8"Проверить обновления", imgui.ImVec2(-1, 32 * scale)) then
				checkUpdates(true)
			end
			imgui.PopStyleVar()
		...
		elseif uiShowLogs[0] then
			...
			if BeginCEFSection("##AddManualLog", u8"Добавить запись", u8"РУЧНОЙ ВВОД", imgui.ImVec2(right_w, 255 * scale), scale) then
				...
				if imgui.Button(u8"Добавить в историю", imgui.ImVec2(-1, 32 * scale)) then
					...
					local p = tonumber((ffi.string(logAddPrice):gsub("[%.,%s]", "")))
					if m ~= "" and p then
						if p >= 10000 then
		...
		elseif uiShowUpdates[0] then
			...
			if BeginCEFSection("##UpdateLogs", u8"Список изменений", u8"ВЕРСИЯ 4.0", imgui.ImVec2(left_w, content_h), scale, 0.0) then
				...
				if updateAvailable then
					imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.2, 0.8, 0.2, 0.1))
					imgui.PushStyleColor(imgui.Col.Border, imgui.ImVec4(0.2, 0.8, 0.2, 0.2))
					imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, auto_update.rounding * scale)
					imgui.BeginChild("##NewUpdateBox", imgui.ImVec2(0, 48 * scale), true)
					
					imgui.SetCursorPos(imgui.ImVec2(10 * scale, 15 * scale))
					imgui.TextColored(imgui.ImVec4(0.4, 0.9, 0.4, 1.0), fa.ICON_FA_EXCLAMATION_CIRCLE .. " " .. u8"Доступно обновление: " .. updateVersion)
					
					local btn_w = 140 * scale
					imgui.SetCursorPos(imgui.ImVec2(imgui.GetWindowWidth() - btn_w - 10 * scale, 10 * scale))
					imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.2, 0.8, 0.2, 0.4))
					imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.2, 0.8, 0.2, 0.6))
					imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.2, 0.8, 0.2, 0.8))
					imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, auto_update.rounding * 0.6 * scale)
					if imgui.Button(fa.ICON_FA_DOWNLOAD .. " " .. u8"Обновить", imgui.ImVec2(btn_w, 28 * scale)) then
						performUpdate(SCRIPT_UPDATE_URL)
					end
					imgui.PopStyleVar()
					imgui.PopStyleColor(3)
...
function se.onShowDialog(id, style, header, but_1, but_2, body)
	...
	if style == 0 and header:find("Подтверждение покупки") then
		...
		local price, comm = body:match("%$([%d%.%,]+)%s*%+.-%$([%d%.%,]+)")
		...
		if price and comm then
			...
			local clean_price = tonumber((price:gsub("[%.,]", "")))
			local clean_comm = tonumber((comm:gsub("[%.,]", "")))
...
function se.onServerMessage(color, text)
	...
	if bought_model and bought_price then
		local price = tonumber((bought_price:gsub("[%.,]", "")))
		if price then
	...
	if listed_model and listed_price then
		listed_model = listed_model:gsub("^транспортное средство ", ""):gsub("^транспорт ", ""):gsub("^свой транспорт ", "")
		pending_sell.active = true
		pending_sell.model = listed_model
		pending_sell.price = tonumber((listed_price:gsub("[%.,]", "")))
	...
	if state_sell_price then
		local price = tonumber((state_sell_price:gsub("[%.,]", "")))
		if price then
	...
	if auc_model and auc_price then
		local price = tonumber((auc_price:gsub("[%.,]", "")))
		if price then
	...
	if sold_model and sold_price then
		local price = tonumber((sold_price:gsub("[%.,]", "")))
		if price then
...
function se.onSendCommand(cmd)
	local price = cmd:match("^/sellcarto %d+ (%d+)")
	if price then
		pending_sell.active = true
		pending_sell.model = "Транспорт (Игроку)"
		pending_sell.price = tonumber(price)
		pending_sell.time = os.clock()
	end
end
	...
	if not parsing.active and button == 1 then
		...
		if input and tonumber(input) then
			...
			if header and (header:find("Продажа") or header:find("Аукцион") or header:find("продажу")) then
				pending_sell.active = true
				pending_sell.price = tonumber(input)
				pending_sell.time = os.clock()
			end
		end
	end
					...
					if model and price then
						...
						if PLATES[str_i] ~= str_v then
							...
							if isCharInArea2d(PLAYER_PED, AB_AREA[1], AB_AREA[2], AB_AREA[3], AB_AREA[4], false) then
								local cefText = string.format("На продажу выставлен: <b>%s</b> за <b>$%s</b><br>", model, price)
								if V[model] ~= nil then
				...
				do -- \\ Выставление транспорта на аукцион
					...
					if model then
						...
						if PLATES[str_i] ~= str_v then
							...
							if isCharInArea2d(PLAYER_PED, AB_AREA[1], AB_AREA[2], AB_AREA[3], AB_AREA[4], false) then
								showCefNotification(string.format("Новый транспорт на аукционе: <b>%s</b>", model), true)
							end
