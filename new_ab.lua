...
local function checkAndDownloadUpdate()
	...
	lua_thread.create(function()
		showCefNotification("Проверка обновлений...", false)
		...
		if response and response ~= "" then
			...
			if status and type(data) == "table" and data.version then
				...
				if new_ver and new_ver > current_ver then
					showCefNotification(string.format("Найдена новая версия: <b>%s</b>. Загрузка...", data.version), false)
					...
					if script_code and script_code ~= "" then
						...
						if file then
							...
							file:close()
							showCefNotification("Обновление загружено! Перезапуск...", false)
							wait(1500)
							thisScript():reload()
						else
							showCefNotification("Ошибка при сохранении файла обновления.", true)
						end
					else
						showCefNotification("Ошибка при скачивании файла обновления.", true)
					end
				else
					showCefNotification("У вас установлена актуальная версия скрипта.", false)
				end
			else
				showCefNotification("Ошибка при обработке данных обновления.", true)
			end
		else
			showCefNotification("Ошибка соединения при проверке обновлений.", true)
		end
		isUpdating = false
	end)
end
...
imgui.OnFrame(function() return renderWindow[0] end, function(player)
	...
	if imgui.Begin("##АвтоБазарCEF", renderWindow, flags) then
		...
		if BeginCEFSection("##Settings", u8"Настройки", u8"ИНТЕРФЕЙС", imgui.ImVec2(right_w, content_h - 305 * scale), scale) then
		...
		elseif uiShowUpdates[0] then
			-- === ОКНО ОБНОВЛЕНИЙ ===
			if BeginCEFSection("##UpdateLogs", u8"Список изменений", u8"ВЕРСИЯ " .. thisScript().version, imgui.ImVec2(left_w, content_h), scale, 0.0) then
				imgui.BeginChild("##UpdatesScroll", imgui.ImVec2(0, 0), false)
				local list_draw_list = imgui.GetWindowDrawList()
				
				imgui.Dummy(imgui.ImVec2(0, 5 * scale))
				imgui.TextColored(imgui.ImVec4(0.4, 0.8, 0.4, 1.0), fa.ICON_FA_CHECK_CIRCLE .. " " .. u8"Версия " .. thisScript().version .. u8" (Текущая)")
				imgui.Dummy(imgui.ImVec2(0, 2 * scale))
			...
			DrawMetric("##upd_ver", u8"ТЕКУЩАЯ ВЕРСИЯ", thisScript().version, imgui.ImVec4(acc[1], acc[2], acc[3], 0.14), right_w, scale)
			imgui.Dummy(imgui.ImVec2(0, 5 * scale))
			...
			imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(acc[1], acc[2], acc[3], 0.8))
			imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, auto_update.rounding * 0.6 * scale)
			if imgui.Button(isUpdating and u8"Проверка..." or u8"Проверить обновления", imgui.ImVec2(right_w, 35 * scale)) then
			...
			if BeginCEFSection("##UpdInfo", u8"Информация", u8"СВЕДЕНИЯ", imgui.ImVec2(right_w, content_h - 125 * scale), scale) then
